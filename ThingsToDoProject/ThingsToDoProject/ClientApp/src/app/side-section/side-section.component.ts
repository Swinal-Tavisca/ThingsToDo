...
export class SideSectionComponent implements OnInit {
  ...
  SetReminder(){
    ...
    this.http.get('http://localhost:50298/api/Data/reminder/' + this.value + '/' + this.response.placeID + '/'+ this.response.name + '/'+ this.response.distance + '/' + this.response.phoneNumber + '/' + "saurabh")
    .subscribe(
...
GetAllDataOfParticularPlace(){
  ...
  let observable = this.http.get('http://localhost:50298/api/Data/place/'+ this.location + '/'+this.PlaceId )
  // .subscribe(
